...
const ONE_PAIR = [];
const HIGH_CARD = [];
const strengthOrder = 'AKQJT98765432J';
...
async function puzzle2() {
  try {
    ...
    for (const bet of Object.keys(hands)) {
      ...
      let finalStore = [];
      if (JCount && JCount.length !== 5) {
        finalStore = totalStore.sort((a, b) => a - b);
        if (JCount.length >= 2) {
          finalStore[finalStore.length - 2] += JCount.length;
          finalStore = finalStore.splice(finalStore.indexOf(JCount.length), 1);
        } else {
          finalStore[finalStore.length - 1] += JCount.length;
          finalStore = finalStore.splice(finalStore.indexOf(JCount.length), 1);
        }
      }
